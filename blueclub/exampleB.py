...
def AccidentReport(count, response):
    ...
    try:
        ...
        # Create a new record
            sql="INSERT INTO acc(CarID, AccDate, AccHour, AccMinute, AccSeconds,OppositeValue, AccLatitude, AccLongitude,Road, City, Town, ModelYear, ModelCode,report) VALUES('"+carID+"','"+accidentdata+"','"+str(hour)+"','"+str(minute)+"','"+str(second)+"','"+oppositevalue+"','"+matrix[count][5]+"','"+matrix[count][6]+"','"+matrix[count][11]+"','"+matrix[count][9]+"','"+matrix[count][10]+"','"+matrix[count][13]+"','"+matrix[count][14]+"','"+response+"')"
            cursor.execute(sql)
